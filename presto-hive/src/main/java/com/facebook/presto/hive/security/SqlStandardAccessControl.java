        ...
        implements ConnectorAccessControl
{
    ...
    private boolean isDatabaseOwner(ConnectorTransactionHandle transaction, Identity identity, String databaseName)
    {
    ...
    private boolean isTableOwner(ConnectorTransactionHandle transaction, Identity identity, SchemaTableName tableName)
    {
        return checkTablePermission(transaction, identity, tableName, OWNERSHIP);
    }

    private boolean checkTablePermission(ConnectorTransactionHandle transaction, Identity identity, SchemaTableName tableName, HivePrivilege requiredPrivilege)
    {
        if (isAdmin(transaction, identity)) {
            return true;
        }
    ...
    private boolean hasGrantOptionForPrivilege(ConnectorTransactionHandle transaction, Identity identity, Privilege privilege, SchemaTableName tableName)
    {
        if (isAdmin(transaction, identity)) {
            return true;
        }
