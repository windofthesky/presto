        ...
        extends ProjectOffPushDownRule<WindowNode>
{
    ...
    protected Optional<PlanNode> pushDownProjectOff(PlanNodeIdAllocator idAllocator, WindowNode windowNode, Set<Symbol> referencedOutputs)
    {
        ...
        for (WindowNode.Frame frame : windowNode.getFrames()) {
            frame.getStartValue().ifPresent(referencedInputs::add);
            frame.getEndValue().ifPresent(referencedInputs::add);
        }

        windowNode.getHashSymbol().ifPresent(referencedInputs::add);

        for (Map.Entry<Symbol, WindowNode.Function> entry : referencedFunctions.entrySet()) {
            referencedInputs.addAll(SymbolsExtractor.extractUnique(entry.getValue().getFunctionCall()));
        }
