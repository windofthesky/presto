...
public class TestWindowOperator
{
    ...
    public void testFindEndPosition()
    {
        ...
        assertEquals(WindowOperator.findEndPosition(1, 1, allPositionsOrdered), 1);
        ...
        assertEquals(WindowOperator.findEndPosition(2, 1, allPositionsOrdered), 2);

        BiFunction<Integer, Integer, Boolean> firstElementDistinct = (p1, p2) -> (p1 == 1 && p2 == 1) || (p1 > 1 && p2 > 1);

        assertEquals(WindowOperator.findEndPosition(1, 2, firstElementDistinct), 1);
        assertEquals(WindowOperator.findEndPosition(1, 3, firstElementDistinct), 1);
        assertEquals(WindowOperator.findEndPosition(1, 4, firstElementDistinct), 1);
        assertEquals(WindowOperator.findEndPosition(1, 10, firstElementDistinct), 1);

        BiFunction<Integer, Integer, Boolean> eachSeparate = (x, y) -> x == y;

        assertEquals(WindowOperator.findEndPosition(1, 2, eachSeparate), 1);
        assertEquals(WindowOperator.findEndPosition(1, 3, eachSeparate), 1);
        assertEquals(WindowOperator.findEndPosition(1, 4, eachSeparate), 1);

        // last element distinct, e.g. X, X, X, Y

        assertEquals(WindowOperator.findEndPosition(1, 2, (x, y) -> true), 2);
        assertEquals(WindowOperator.findEndPosition(1, 3, (x, y) -> (x == 2 && y == 2) || (x < 2 && y < 2)), 2);
        assertEquals(WindowOperator.findEndPosition(1, 4, (x, y) -> (x == 3 && y == 3) || (x < 3 && y < 3)), 3);
        assertEquals(WindowOperator.findEndPosition(1, 10, (x, y) -> (x == 9 && y == 9) || (x < 9 && y < 9)), 9);
        ...
        assertEquals(WindowOperator.findEndPosition(1, 2, allEqualButZeroth), 1);
        assertEquals(WindowOperator.findEndPosition(1, 3, allEqualButZeroth), 1);
        assertEquals(WindowOperator.findEndPosition(1, 4, allEqualButZeroth), 1);
    }
