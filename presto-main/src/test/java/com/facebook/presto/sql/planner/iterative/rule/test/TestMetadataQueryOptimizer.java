...
public class TestMetadataQueryOptimizer
{
    ...
    private static final Signature COUNT_SIGNATURE = new Signature(
            "count",
    ...
    public void setUp()
    {
        ...
        tester = new RuleTester(queryRunner, session);
    ...
    public void testDoesNotFireWhenNotAnAllowedFunction()
    {
        ...
        Signature countSignature = new Signature(
                "count",
                FunctionKind.AGGREGATE,
                ImmutableList.of(),
                ImmutableList.of(),
        ...
        tester.assertThat(metadataQueryOptimizer)
                .on(p -> p.aggregate(
                        ImmutableMap.of(p.symbol("COUNT", BIGINT),
                                new Aggregation(countFunctionCall, countSignature, Optional.empty())),
                        ImmutableList.of(ImmutableList.of(new Symbol("key"))),
    ...
    public void testDoesNotFireWhenProjectionIsNonDeterministic()
    {
        tester.assertThat(metadataQueryOptimizer)
                .on(p -> p.aggregate(
                        ImmutableMap.of(p.symbol("COUNT", BIGINT),
                                new Aggregation(COUNT_DISTINCT_FUNCTION_CALL, COUNT_SIGNATURE, Optional.empty())),
                        ImmutableList.of(ImmutableList.of(p.symbol("key", BIGINT))),
                        p.project(
                                Assignments.of(
        ...
        tester.assertThat(metadataQueryOptimizer)
                .on(p -> p.aggregate(
                        ImmutableMap.of(p.symbol("COUNT", BIGINT),
                                new Aggregation(COUNT_DISTINCT_FUNCTION_CALL, COUNT_SIGNATURE, Optional.empty())),
                        ImmutableList.of(ImmutableList.of(p.symbol("key", BIGINT))),
                        p.filter(p.expression("key > 1"), p.tableScan(ImmutableList.of(p.symbol("key", BIGINT))))))
                .doesNotFire();
    }
    ...
    public void testMetadataQueryOptimization()
    {
        ...
        tester.assertThat(metadataQueryOptimizer)
                .on(p -> p.aggregate(
                        ImmutableMap.of(p.symbol("COUNT", BIGINT), new Aggregation(COUNT_DISTINCT_FUNCTION_CALL, COUNT_SIGNATURE, Optional.empty())),
