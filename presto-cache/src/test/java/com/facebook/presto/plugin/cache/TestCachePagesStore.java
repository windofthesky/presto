...
public class TestCachePagesStore
{
    public static final ConnectorSession SESSION = new TestingConnectorSession(ImmutableList.of());

    private CachePagesStore pagesStore;
    private CachePageSinkProvider pageSinkProvider;

    @BeforeMethod
    public void setUp()
    {
        pagesStore = new CachePagesStore(new CacheConfig().setMaxDataPerNode(new DataSize(1, DataSize.Unit.MEGABYTE)));
        pageSinkProvider = new CachePageSinkProvider(pagesStore);
    }
    ...
    private void insertToTable(long tableId, Page page, Long... activeTableIds)
    {
        ConnectorPageSink pageSink = pageSinkProvider.createPageSink(
                CacheTransactionHandle.INSTANCE,
                SESSION,
                createCacheInsertTableHandle(tableId, activeTableIds));
    ...
    private void createTable(long tableId, Long... activeTableIds)
    {
        ConnectorPageSink pageSink = pageSinkProvider.createPageSink(
                CacheTransactionHandle.INSTANCE,
                SESSION,
                createCacheOutputTableHandle(tableId, activeTableIds));
        pageSink.finish();
    }

    private static ConnectorOutputTableHandle createCacheOutputTableHandle(long tableId, Long... activeTableIds)
    {
        return new CacheOutputTableHandle(
                new CacheTableHandle(
                        "test",
                        "schema",
    ...
    private static ConnectorInsertTableHandle createCacheInsertTableHandle(long tableId, Long[] activeTableIds)
    {
        return new CacheInsertTableHandle(
                new CacheTableHandle(
                        "test",
                        "schema",
                        format("table_%d", tableId),
