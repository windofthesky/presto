        ...
        implements PlanOptimizer
{
    ...
    public DetermineJoinDistributionType(StatsCalculator statsCalculator, GlobalProperties globalProperties, int nodeCount)
    {
        this.statsCalculator = requireNonNull(statsCalculator, "statisticsCalculator is null");
    ...
    public PlanNode optimize(PlanNode plan, Session session, Map<Symbol, Type> types, SymbolAllocator symbolAllocator, PlanNodeIdAllocator idAllocator)
    {
        ...
        return SimplePlanRewriter.rewriteWith(new Rewriter(session, globalProperties, statsCalculator, types, nodeCount), plan);
    }
            ...
            extends SimplePlanRewriter<Void>
    {
        ...
        public Rewriter(Session session, GlobalProperties globalProperties, StatsCalculator statsCalculator, Map<Symbol, Type> types, int nodeCount)
        {
        ...
        private boolean isSmall(PlanNode node)
        {
            ...
            double dataSize = statsCalculator.calculateStats(node, noLookup(), session, types).getOutputRowCount() * ROW_SIZE_ESTIMATE;
            return !isNaN(dataSize) && dataSize < smallSizeLimit;
        }

        private boolean isMuchSmaller(PlanNode node, PlanNode other)
        {
            double nodeDataSize = statsCalculator.calculateStats(node, noLookup(), session, types).getOutputRowCount() * ROW_SIZE_ESTIMATE;
            double otherDataSize = statsCalculator.calculateStats(other, noLookup(), session, types).getOutputRowCount() * ROW_SIZE_ESTIMATE;
            return !isNaN(nodeDataSize) && !isNaN(otherDataSize) && nodeDataSize * nodeCount < otherDataSize;
        }
