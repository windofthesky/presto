        ...
        implements CostCalculator
{
    ...
    @Override
    public PlanNodeCostEstimate calculateCost(Session session, Map<Symbol, Type> types, PlanNode planNode, Lookup lookup)
    {
        ...
        return costCalculator.calculateCost(session, types, planNode, lookup).add(estimatedExchangeCost);
    }
            ...
            extends PlanVisitor<Void, PlanNodeCostEstimate>
    {
        ...
        {
            return lookup.getCumulativeCost(session, types, node);
        }
    }
}
