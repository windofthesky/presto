...
import static com.facebook.presto.plugin.cache.CacheErrorCode.CACHE_LIMIT_EXCEEDED;
import static com.facebook.presto.plugin.cache.CacheErrorCode.MISSING_DATA;
import static java.lang.String.format;

@ThreadSafe
public class CachePagesStore
{
    ...
    @Inject
    public CachePagesStore(CacheConfig config)
    {
    ...
    public synchronized void add(Long tableId, Page page)
    {
        ...
        if (maxBytes < newSize) {
            throw new PrestoException(CACHE_LIMIT_EXCEEDED, format("Cache limit [%d] for cache connector exceeded", maxBytes));
        }
    ...
    public synchronized void cleanUp(Set<Long> activeTableIds)
    {
        ...
        // That can lead to a situation when CachePagesStore already knows about a newer second table on some worker
        // but cleanUp is triggered by insert from older first table, which CacheTableHandle was created before
