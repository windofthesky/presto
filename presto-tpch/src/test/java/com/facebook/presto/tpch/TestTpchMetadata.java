...
public class TestTpchMetadata
{
    ...
    private void testColumnStats(String schema, TpchTable<?> table, TpchColumn<?> column, RangeColumnStatistics expectedStatistics)
    {
        testColumnStats(schema, table, column, alwaysTrue(), expectedStatistics);
    }

    private void testColumnStats(String schema, TpchTable<?> table, TpchColumn<?> column, Constraint<ColumnHandle> constraint, RangeColumnStatistics expected)
    {
        TpchTableHandle tableHandle = tpchMetadata.getTableHandle(session, new SchemaTableName(schema, table.getTableName()));
        TableStatistics tableStatistics = tpchMetadata.getTableStatistics(session, tableHandle, constraint);
        ColumnHandle columnHandle = tpchMetadata.getColumnHandles(session, tableHandle).get(column.getSimplifiedColumnName());

        RangeColumnStatistics actual = tableStatistics.getColumnStatistics().get(columnHandle).getOnlyRangeColumnStatistics();

        EstimateAssertion estimateAssertion = new EstimateAssertion(TOLERANCE);

        estimateAssertion.assertClose(actual.getDistinctValuesCount(), expected.getDistinctValuesCount(), "distinctValuesCount-s differ");
        estimateAssertion.assertClose(actual.getDataSize(), expected.getDataSize(), "dataSize-s differ");
        estimateAssertion.assertClose(actual.getNullsFraction(), expected.getNullsFraction(), "nullsFraction-s differ");
        estimateAssertion.assertClose(actual.getLowValue(), expected.getLowValue(), "lowValue-s differ");
        estimateAssertion.assertClose(actual.getHighValue(), expected.getHighValue(), "highValue-s differ");
    }
    ...
    private RangeColumnStatistics columnStatistics(double distinctValuesCount)
    {
        return createColumnStatistics(Optional.of(distinctValuesCount), empty(), empty());
    }

    private RangeColumnStatistics columnStatistics(double distinctValuesCount, String min, String max)
    {
        return createColumnStatistics(Optional.of(distinctValuesCount), Optional.of(utf8Slice(min)), Optional.of(utf8Slice(max)));
    }

    private RangeColumnStatistics columnStatistics(double distinctValuesCount, double min, double max)
    {
        return createColumnStatistics(Optional.of(distinctValuesCount), Optional.of(min), Optional.of(max));
    }

    private RangeColumnStatistics rangeStatistics(String min, String max)
    {
        return createColumnStatistics(empty(), Optional.of(utf8Slice(min)), Optional.of(utf8Slice(max)));
    }

    private RangeColumnStatistics rangeStatistics(double min, double max)
    {
        return createColumnStatistics(empty(), Optional.of(min), Optional.of(max));
    }

    private RangeColumnStatistics unknownStatistics()
    {
        return createColumnStatistics(empty(), empty(), empty());
    }

    private RangeColumnStatistics createColumnStatistics(Optional<Double> distinctValuesCount, Optional<Object> min, Optional<Object> max)
    {
