...
public final class GroupingOperationRewriter
{
    ...
    public static Expression rewriteGroupingOperation(GroupingOperation expression, QuerySpecification queryNode, Analysis analysis, Metadata metadata, Optional<Symbol> groupIdSymbol)
    {
        ...
        if (isSimpleGroupByPresent) {
            ...
            List<Expression> groupingOrdinals = expression.getGroupingColumns().stream()
                    .map(groupingColumn -> extractGroupingColumnOrdinal(groupingColumn, columnReferences, columnReferenceFields))
            ...
            for (List<Expression> groupingSet : analysis.getGroupingSets(queryNode)) {
                groupingSetOrdinalsBuilder.add(
                        groupingSet.stream()
                            .map(groupingColumn -> extractGroupingColumnOrdinal(groupingColumn, columnReferences, columnReferenceFields))
                            .map(columnOrdinal -> new LongLiteral(Integer.toString(columnOrdinal)))
                            .collect(toImmutableList()));
    ...
    // TODO: This method is a hack and is needed until Analysis#columnReferences is converted to a IdentityLinkedHashMap.
    // The first line fetches the columnReference instance using equality. That instance is then used to lookup the FieldId
    // using an identity comparison.
    private static int extractGroupingColumnOrdinal(Expression groupingExpression, List<Expression> columnReferences, Map<Expression, FieldId> columnReferenceFields)
    {
        Expression lookupExpression = columnReferences.get(columnReferences.indexOf(groupingExpression));
        return columnReferenceFields.get(lookupExpression).getFieldIndex();
    }
}
