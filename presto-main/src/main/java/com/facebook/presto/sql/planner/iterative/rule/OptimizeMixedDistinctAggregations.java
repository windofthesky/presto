        ...
        implements Rule
{
    ...
    public Optional<PlanNode> apply(PlanNode node, Lookup lookup, PlanNodeIdAllocator idAllocator, SymbolAllocator symbolAllocator, Session session)
    {
        ...
        Set<Symbol> masks = getAggregationMask(aggregationNode);
        if (masks.size() != 1 || aggregationNode.getMasks().size() == aggregationNode.getAggregations().size()) {
            return Optional.empty();
        }

        if (aggregationNode.getAggregations().values().stream().map(FunctionCall::getFilter).anyMatch(Optional::isPresent)) {
        ...
        for (Map.Entry<Symbol, Aggregation> entry : aggregationNode.getAssignments().entrySet()) {
            FunctionCall functionCall = entry.getValue().getCall();
            if (functionCall.isDistinct()) {
                assignments.put(entry.getKey(), new Aggregation(new FunctionCall(
    ...
    private static Set<Symbol> getAggregationMask(AggregationNode aggregationNode)
    {
        return ImmutableSet.copyOf(aggregationNode.getMasks().values());
    }

    private static Symbol getDistinctAggregationMask(AggregationNode aggregationNode)
    {
        return Iterables.getOnlyElement(getAggregationMask(aggregationNode));
    }
            ...
            PlanNodeIdAllocator idAllocator)
    {
        ...
        List<Symbol> nonDistinctAggregateSymbols = getOriginalNonDistinctAggregateArgs(aggregationNode.getAssignments()); //b
        Symbol distinctSymbol = Iterables.getOnlyElement(getOriginalDistinctAggregateArgs(aggregationNode.getAssignments())); // c
    ...
    {
        for (Symbol symbol : getOriginalNonDistinctAggregateArgs(aggregationNode.getAssignments())) {
