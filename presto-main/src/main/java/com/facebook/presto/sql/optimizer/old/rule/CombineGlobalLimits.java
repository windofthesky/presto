...
public class CombineGlobalLimits
        implements Rule
{
    @Override
    ...
    private Expression process(GlobalLimit parent, GlobalLimit child)
    {
        if (parent.getCount() < child.getCount()) {
            return new GlobalLimit(parent.getCount(), child.getArguments().get(0));
        }
        return child;
    }
}
