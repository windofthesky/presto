...
public class TestDetermineJoinDistributionType
{
    ...
    private void assertPlan(Session session, @Language("SQL") String sql, PlanMatchPattern pattern)
    {
        List<PlanOptimizer> optimizers = ImmutableList.of(
                ...
                new IterativeOptimizer(new StatsRecorder(), statsCalculator, new CostCalculatorWithEstimatedExchanges(new CostCalculatorUsingExchanges(1), 1), ImmutableSet.of(new RemoveRedundantIdentityProjections())),
                new TransformUncorrelatedInPredicateSubqueryToSemiJoin(),
                new DetermineJoinDistributionType(statsCalculator, new GlobalProperties(), 1));
        queryRunner.inTransaction(session, transactionSession -> {
            Plan actualPlan = queryRunner.createPlan(transactionSession, sql, optimizers);
            PlanAssert.assertPlan(transactionSession, queryRunner.getMetadata(), Lookup.noLookup(), actualPlan, pattern);
            return null;
        });
    }
}
