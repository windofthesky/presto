        ...
        implements Rule
{
    ...
    public Optional<PlanNode> apply(PlanNode node, Lookup lookup, PlanNodeIdAllocator idAllocator, SymbolAllocator symbolAllocator, Session session)
    {
        ...
        for (FunctionCall call : aggregation.getAggregations().values()) {
            if (!ALLOWED_FUNCTIONS.contains(call.getName().toString()) && !call.isDistinct()) {
                return Optional.empty();
            }
        }

        Optional<TableScanNode> result = findTableScan(lookup.resolve(aggregation.getSource()), lookup);
        if (!result.isPresent()) {
            return Optional.empty();
        }
        ...
        PlanNode replacedNode = searchFrom(aggregation, lookup)
