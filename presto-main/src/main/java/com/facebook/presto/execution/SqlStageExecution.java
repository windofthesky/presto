...
public final class SqlStageExecution
{
    ...
    private boolean dynamicFilterSchedulingInfoPropagated;
            ...
            DynamicFilterService dynamicFilterService)
    {
        ...
        addStateChangeListener(newState -> {
            if (!dynamicFilterSchedulingInfoPropagated &&
                    (newState == SCHEDULING_SPLITS || newState == SCHEDULED || newState == RUNNING)) {
        ...
        for (PlanNode node : nodes) {
            if (node instanceof JoinNode) {
                ...
                for (TaskId task : allTasks) {
                    dynamicFilterService.registerTask(stateMachine.getSession().getQueryId().toString(),
                            joinNode.getId().toString(),
                            stateMachine.getStageId().getId(),
                            task.getId());
                }
            }
